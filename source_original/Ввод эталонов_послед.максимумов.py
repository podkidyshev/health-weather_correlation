...
print("Пациент 1_5:", xr5, len(xr5),"выборочное среднее =", np.mean(xr5), "стандартное отклонение =", np.std(xr5), "\n", "радиус интервала =", 2.09*np.std(xr5)/sqrt(len(xr5)-1), "Доверительный интервал:", [np.mean(xr5)- 2.09*np.std(xr5)/sqrt(len(xr5)-1), np.mean(xr5) + 2.09*np.std(xr5)/sqrt(len(xr5)-1)])
xr6 = sequence_distance(sequence_max(data6), sequence_max(data))
print("Пациент 1_6:", xr6, len(xr6),"выборочное среднее =", np.mean(xr6), "стандартное отклонение =", np.std(xr6), "\n", "радиус интервала =", 2.14*np.std(xr6)/sqrt(len(xr6)-1), "Доверительный интервал:", [np.mean(xr6)- 2.14*np.std(xr6)/sqrt(len(xr6)-1), np.mean(xr6) + 2.14*np.std(xr6)/sqrt(len(xr6)-1)])
...
vol = sum(sequence_max(data1)) + sum(sequence_max(data2)) + sum(sequence_max(data3)) + sum(sequence_max(data4)) + sum(sequence_max(data5)) + sum(sequence_max(data6)) 
print("Сгруппированное распределение расстояний от максимумов первой группы пациентов без нагрузки до ближайшего максимума Kp")
print(raspred(xr_group), "объем выборки =",  vol, "\n", "радиус интервала =", 1.96*np.std(xr_group)/sqrt(vol),  "Доверительный интервал:", [np.mean(xr_group) - 1.96*np.std(xr_group)/sqrt(vol), np.mean(xr_group) + 1.96*np.std(xr_group)/sqrt(vol)])

#pn=norm.cdf(x, 0, 1)#нормальное интегральное  распределение 
#ax.plot(x,pn, lw=5, alpha=0.6, label='norm cdf')
pi=norm.pdf(x, 0.6, 1.6)
ax.plot(x,pi, lw=5, alpha=0.6, label='norm pdf')
