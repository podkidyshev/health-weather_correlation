...
for i in range(n_standart):
    m_list = [] 
    for j in range(len(sample_n)): m_list.append(np.mean(sequence_distance(sequence_max(sample_n[j]), sequence_max(standart[i])))) 
...
for i in range(n_standart):
    m_list = [] 
    for j in range(len(sample_e)): m_list.append(np.mean(sequence_distance(sequence_max(sample_e[j]), sequence_max(standart[i])))) 
...
for i in range(n_standart):
    m_list = [] 
    for j in range(len(sample_o)): m_list.append(np.mean(sequence_distance(sequence_max(sample_o[j]), sequence_max(standart[i])))) 
    max_sample_list_o.append(m_list)
print("Распределение средних значений пациентов после отдыха для всех эталонов", "\n", max_sample_list_o)

print("Результаты статистического группового анализа  по средним значениям образцов для всех факторов и для всех эталонов")
...
for i in range(n_standart):
    ...
    print("Результаты группового анализа распределения средних значений всех образцов с физической нагрузкой для эталона  ", i, "\n", "[Выборочное среднее, Стандартное отклонение,  Доверительный интервал] =  ", "\n", stat_analys(max_sample_list_n[i]))
    print("Результаты группового анализа распределения средних значений всех образцов после отдыха для эталона  ", i, "\n", "[Выборочное среднее, Стандартное отклонение,  Доверительный интервал] =  ", "\n", stat_analys(max_sample_list_o[i]))
    print("Результаты группового анализа распределения средних значений всех образцов с эмоциональной нагрузкой для эталона  ", i, "\n", "[Выборочное среднее, Стандартное отклонение,  Доверительный интервал] =  ", "\n", stat_analys(max_sample_list_e[i]))


    
print("Результаты  визуального  анализа для всех эталонов группового фактор-образца «без нагрузки» и распределения средних значений группы для фактор-образца «без нагрузки»")
       
for i in range(n_standart):
    print("Результаты визуального анализа распределения группового фактор-образца «без нагрузки»  для эталона ", i, "\n", visual_analys2(sequence_distance(sequence_max(sum_list(sample)), sequence_max(standart[i])), max_sample_list[i]))  
    print("Результаты тестирования нормальности распределения группового фактор-образца «без нагрузки» для эталона ", i, "\n")
    test_normal(sequence_distance(sequence_max(sum_list(sample)), sequence_max(standart[i])))
    print("Результаты тестирования нормальности распределения средних значений группы для фактор-образца без нагрузки для эталона ", i, "\n")
    test_normal(max_sample_list[i])

print("Результаты статистического группового анализа по дням и по средним значениям для всех образцов без нагрузки для всех эталонов")
