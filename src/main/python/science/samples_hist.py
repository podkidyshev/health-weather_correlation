...
with open("samples/Flow_62.txt") as file:
...
print("Анализ распределений расстояний пациента 1_1 без нагрузки:", "\n", "выборочное среднее =", np.mean(xr1),
      "стандартное отклонение =", np.std(xr1), "\n", "Доверительный интервал:",
      st.t.interval(0.95, len(xr1) - 1, loc=np.mean(xr1), scale=st.sem(xr1)))

print("Анализ распределений расстояний пациента 1_1 с физической нагрузкой:", "\n", "выборочное среднее =",
      np.mean(xr2), "стандартное отклонение =", np.std(xr2), "\n", "Доверительный интервал:",
      st.t.interval(0.95, len(xr2) - 1, loc=np.mean(xr2), scale=st.sem(xr2)))

print("Анализ распределений расстояний пациента 1_1 после отдыха:", "\n", "выборочное среднее =", np.mean(xr3),
      "стандартное отклонение =", np.std(xr3), "\n", "Доверительный интервал:",
      st.t.interval(0.95, len(xr3) - 1, loc=np.mean(xr3), scale=st.sem(xr3)))

print("Анализ распределений расстояний пациента 1_1 с эмоциональной нагрузкой:", "\n", "выборочное среднее =",
      np.mean(xr4), "стандартное отклонение =", np.std(xr4), "\n", "Доверительный интервал:",
      st.t.interval(0.95, len(xr4) - 1, loc=np.mean(xr4), scale=st.sem(xr4)))
