# -*- coding: utf-8 -*-
# Ввод образцов_послед.максимумов_распред..расстояний
from numpy import *
...
with open("samples/1_1n.txt") as file:
...
with open("samples/1_1o.txt") as file:
...
with open("samples/1_1e.txt") as file:
...
print("Список максимумов значений пациента 1_1 с эмоциональной нагрузкой:", sequence_max(data4),
      len(sequence_max(data4)))
...
print("Анализ распределений расстояний пациента 1_1 без нагрузки", "выборочное среднее =", np.mean(xr1),
      "стандартное отклонение =", np.std(xr1), "\n", "радиус интервала =", 2.10 * np.std(xr1) / sqrt(len(xr1) - 1),
      "Доверительный интервал:",
      [np.mean(xr1) - 2.10 * np.std(xr1) / sqrt(len(xr1) - 1), np.mean(xr1) + 2.10 * np.std(xr1) / sqrt(len(xr1) - 1)])

print("Анализ распределений расстояний пациента 1_1 с физической нагрузкой:", "выборочное среднее =", np.mean(xr2),
      "стандартное отклонение =", np.std(xr2), "\n", "радиус интервала =", 2.13 * np.std(xr2) / sqrt(len(xr2) - 1),
      "Доверительный интервал:",
      [np.mean(xr2) - 2.13 * np.std(xr2) / sqrt(len(xr2) - 1), np.mean(xr2) + 2.13 * np.std(xr2) / sqrt(len(xr2) - 1)])

print("Анализ распределений расстояний пациента 1_1 после отдыха:", "выборочное среднее =", np.mean(xr3),
      "стандартное отклонение =", np.std(xr3), "\n", "радиус интервала =", 2.09 * np.std(xr3) / sqrt(len(xr3) - 1),
      "Доверительный интервал:",
      [np.mean(xr3) - 2.09 * np.std(xr3) / sqrt(len(xr3) - 1), np.mean(xr3) + 2.09 * np.std(xr3) / sqrt(len(xr3) - 1)])

print("Анализ распределений расстояний пациента 1_1 с эмоциональной нагрузкой:", "выборочное среднее =", np.mean(xr4),
      "стандартное отклонение =", np.std(xr4), "\n", "радиус интервала =", 2.18 * np.std(xr4) / sqrt(len(xr4) - 1),
      "Доверительный интервал:",
