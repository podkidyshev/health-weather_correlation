...
def visual_analysis(x):
    ...
    fig, ax = plt.subplots()
    _range = np.linspace(-1,2, 100)
    plt.style.use('seaborn-white')
    ax.hist(x, bins=18,  range  = (-1,2), normed=True, alpha=0.5,
...
"Построение 4-х ядерных оценок плотности и кривой Гаусса для пациентов и эталонов"
...
#Распределение средних значений пациентов без нагрузки для всех эталонов"
max_sample_list = []
for i in range(n_standart):
    m_list = [] 
...
#Распределение средних значений пациентов с физической нагрузкой для всех эталонов"
max_sample_list_n = []
for i in range(n_standart):
    m_list = [] 
...
#Распределение средних значений пациентов с эмоциональной нагрузкой для всех эталонов"
max_sample_list_e = []
for i in range(n_standart):
    m_list = [] 
...
#Распределение средних значений пациентов после отдыха для всех эталонов"
max_sample_list_o = []
for i in range(n_standart):
    m_list = [] 
...
print("Результаты  визуального  группового анализа по дням и по средним значениям для всех образцов без нагрузки и всех эталонов")
...
for i in range(n_standart):
    ...
    print("Результаты тестирования нормальности распределения для сгруппированным по дням образцов без нагрузки для эталона ", i, "\n")
    test_normal(sequence_distance(sequence_max(sum_list(sample)), sequence_max(standart[i])))
    print("Результаты тестирования нормальности распределения средних значений группы всех образцов без нагрузки для эталона ", i, "\n")
    test_normal(max_sample_list[i])

print("Результаты статистического группового анализа по дням и по средним значениям для всех образцов без нагрузки всех образцов")
