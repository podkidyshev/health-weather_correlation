...
def visual_analysis1(x):
    """Визуальный анализ ряда распределений"""
    fig, ax = plt.subplots()
    _range = np.linspace(-3,4, 100)
    plt.style.use('seaborn-white')
    ax.hist(x, bins=30,  range  = (-3,4), normed=True, alpha=0.5,
    histtype='stepfilled', color='steelblue', edgecolor='none')
    ax.plot(_range, norm.pdf(_range, np.mean(x), np.std(x)))
    ax.plot(_range, st.gaussian_kde(x)(_range))
    return plt.show() 
...
m_list = [] 
for j in range(len(sample)): m_list.append(np.mean(sequence_distance(sequence_max(sample[j]), sequence_max(standart[0])))) 
#print(m_list)    
max_sample_list = []
for i in range(n_standart):
    m_list = [] 
